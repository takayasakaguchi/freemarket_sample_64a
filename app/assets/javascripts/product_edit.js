...
// 新規投稿フォーム
  $('#image-box_edit').on('change', '.js-file', function(e) {
    const targetIndexNew = $(this).parent().data('index');
    // ファイルのブラウザ上でのURLを取得する
    const file = e.target.files[0];
    const blobUrl = window.URL.createObjectURL(file);
    ...
    if (img = $(`img[data-index="${targetIndexNew}"]`)[0]) {
      img.setAttribute('src', blobUrl);
    } else if ($('.uploaded_img').length <= 10){  // 新規画像追加の処理
      $('.uploaded_images').append(buildImg(targetIndexNew, blobUrl));
      // fileIndexの先頭の数字を使ってinputを作る
      $('#image-box__container_edit').append(buildFileField([fileIndex.length - 1] + 1));
      fileIndex.shift();
      // 末尾の数に1足した数を追加する
      fileIndex.push(fileIndex[fileIndex.length - 1] + 1);
    } 
  ...
  $('#image-box_edit').on('click', '.js-remove_new', function() {
    targetIndexNew = $(this).parent().data('index');
    // 該当indexを振られているチェックボックスを取得する
